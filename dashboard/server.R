...
grafPrimSegHanual <- ggplot(tablaPrimSegHA, aes(x=años, y=nSegsPregs, fill=segGenero)) +
    geom_bar(position = "dodge", stat = "identity") +
    scale_fill_manual(values=c("#80FF92", "#4590FF")) +
    theme_minimal() + 
...
grafPrimSegManual <- ggplot(tablaPrimSegMA, aes(x=años, y=nSegsPregs, fill=segGenero)) +
    geom_bar(position = "dodge", stat = "identity") +
    scale_fill_manual(values=c("#80FF92", "#4590FF")) +
    theme_minimal() + 
    theme(text = element_text(size = 9)) +
...
server <- function(input, output) {
  ...
  output$grafPrimSegHui <- renderPlotly({ggplotly(grafPrimSegHanual)})
  
  output$GrafPrimSegMui <- renderPlotly({ggplotly(grafPrimSegManual)})
